...
import {ERC4626NonCompliantStrategyVault} from "@src/strategies/ERC4626NonCompliantStrategyVault.sol";
import {IERC4626} from "@openzeppelin/contracts/interfaces/IERC4626.sol";
import {SafeERC20} from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import {Auth} from "@src/Auth.sol";
import {BaseScript} from "@script/BaseScript.s.sol";

contract ERC4626NonCompliantStrategyVaultScript is BaseScript {
    ...
    function deploy(Auth auth_, uint256 firstDepositAmount_, IERC4626 vault_)
        public
        returns (ERC4626NonCompliantStrategyVault erc4626NonCompliantStrategyVault)
    {
        string memory name = string.concat("Size ", vault_.name(), " Strategy Vault");
        string memory symbol = string.concat("sz", vault_.symbol());
        address implementation = address(new ERC4626NonCompliantStrategyVault());
        ...
        erc4626NonCompliantStrategyVault =
            ERC4626NonCompliantStrategyVault(create2Deployer.computeAddress(salt, keccak256(creationCode)));
        IERC20Metadata(address(vault_.asset())).forceApprove(
            address(erc4626NonCompliantStrategyVault), firstDepositAmount_
        );
