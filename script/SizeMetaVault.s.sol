...
contract SizeMetaVaultScript is Script {
    ...
    function deploy(Auth auth_, IERC20Metadata asset_, uint256 firstDepositAmount_, address[] memory strategies_)
        public
        returns (SizeMetaVault sizeMetaVault)
    {
        ...
        bytes memory initializationData =
            abi.encodeCall(SizeMetaVault.initialize, (auth_, asset_, name, symbol, firstDepositAmount_, strategies_));
        ...
        IERC20(address(asset_)).forceApprove(address(sizeMetaVault), firstDepositAmount_);
        Create2.deploy(
