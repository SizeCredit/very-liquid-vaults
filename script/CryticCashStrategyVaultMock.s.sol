...
contract CryticCashStrategyVaultMockScript is Script {
    ...
    function deploy(SizeVault sizeVault_) public returns (CryticCashStrategyVaultMock) {
        return CryticCashStrategyVaultMock(
            address(
                new ERC1967Proxy(
                    address(new CryticCashStrategyVaultMock()),
                    abi.encodeCall(
                        ...
                        (
                            ...
                            string.concat(
                                "sizeCryticCash", IERC20Metadata(address(sizeVault_.asset())).symbol(), "MOCK"
                            )
                        )
                    )
                )
