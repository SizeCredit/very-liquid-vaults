...
contract VeryLiquidVaultScript is BaseScript {
    ...
    function run() public {
        ...
        console.log(
            "VeryLiquidVault", address(deploy(identifier, auth, asset, veryLiquidVaultFirstDepositAmount, strategies))
        );
    ...
    function deploy(
        ...
        uint256 veryLiquidVaultFirstDepositAmount_,
        IVault[] memory strategies_
    ) public returns (VeryLiquidVault veryLiquidVault) {
        ...
        bytes memory initializationData = abi.encodeCall(
            VeryLiquidVault.initialize,
            (auth_, asset_, name, symbol, fundingAccount, veryLiquidVaultFirstDepositAmount_, strategies_)
        );
