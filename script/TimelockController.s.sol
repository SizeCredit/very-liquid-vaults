...
contract TimelockControllerScript is Script {
  ...
  function run() public {
    ...
    TimelockController timelockController_DEFAULT_ADMIN_ROLE = new TimelockController(7 days, adminTimelockAddresses, adminTimelockAddresses, msg.sender);
    ...
    TimelockController timelockController_VAULT_MANAGER_ROLE = new TimelockController(1 days, vaultManagerAddresses, vaultManagerAddresses, msg.sender);
  ...
  function deploy(uint256 minDelay_, address[] memory proposers_, address[] memory executors_, address admin_) public returns (TimelockController) {
