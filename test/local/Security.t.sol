...
contract SecurityTest is BaseTest {
    function test_Security_deposit_directly_to_strategy_should_not_steal_assets() public {
        ...
        console.log(
            "[before inflating] pricePerShare: %e", veryLiquidVault.totalAssets() * 1e6 / veryLiquidVault.totalSupply()
        );
        erc4626StrategyVault.deposit(depositAmount, attacker); // deposit into underlying strategy to inflate very liquid share price
        console.log(
            "[after inflating] pricePerShare: %e", veryLiquidVault.totalAssets() * 1e6 / veryLiquidVault.totalSupply()
        );
    ...
    function test_Security_fee_minting_uses_correct_share_conversion() public {
        ...
        assertEq(
            Math.mulDiv(veryLiquidVault.totalAssets(), 1e18, veryLiquidVault.totalSupply()), 1e18, "PPS should be 1"
        );
        ...
        assertEq(
            Math.mulDiv(veryLiquidVault.totalAssets(), 1e18, veryLiquidVault.totalSupply()), 2e18, "PPS should be 2"
        );
